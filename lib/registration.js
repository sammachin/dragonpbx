  ...
  async register() {
    ...
    try {
      ...
      if (contacts.includes(this.callID) || contacts.length < this.maxClients) {
        this.logger.info('Updating/adding existing registration')
      } else {
      ...
      await this.rclient.multi()
        ...
        .execAsPipeline()
      let allContacts = await this.getClientContacts(key, true)
      this.res.send(200, {headers: {expires: expires, contact: allContacts}})
    } catch (error) {
      this.logger.error(error)
      this.res.send(500)
    }
