...
async function httpRequest(logger, uri, req) {
  ...
  try {
    const body = {
      ...
      // Set for a REST update-leg fetch so the app can distinguish it from a
      // normal inbound call hook; absent for ordinary calls.
      ...(req.locals.trigger ? { trigger: req.locals.trigger } : {}),
      ...(req.locals.leg ? { leg: req.locals.leg } : {})
    };
...
const getCallHook = async(req, res, next) => {
  ...
  if (req.locals.trunk){
    ...
    logger.info(dialplan, 'Using trunk dialplan');
  } else{
  ...
  if (!dialplan){
    logger.error(`No dialplan found`)
    res.send(404)
  } else {
    logger.info(`Fetching callHook from: dialplan`);
    let callHook = false
    const dialed = req.locals.toUri.user
    // Prefer an exact, literal key match so dial codes containing glob-special
    // characters (e.g. *8, #1) are honoured. Only fall back to glob matching
    // when there's no exact key, which is what gives "*" its catch-all meaning.
    if (Object.prototype.hasOwnProperty.call(dialplan, dialed)) {
      callHook = dialplan[dialed]
    } else {
      // nocomment so a pattern beginning with # isn't treated as a glob comment.
      Object.keys(dialplan).forEach((i) => {
        if (matcher(dialed, [i], {nocomment: true}).length == 1){
          callHook =  dialplan[i]
        }
      })
    }
...
const getCallScript = async(req, res, next) => {
  ...
  if (script){
    ...
    logger.debug(script, `Got CallScript`)
    if (next){
      next()
    } else {
      return
