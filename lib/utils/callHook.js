...
const qs = require('qs');
const { getCallHook } = require('../data/json/lookup');
...
function ensureArray(input) {
  ...
  // Otherwise, wrap the input in an array
  return [input];
}

    const callHook = async(req, res, next) => {
        const {logger} = req.srf.locals;
        registration_hook = await getCallHook(req.locals.domain, req.locals.fromUri.user )
