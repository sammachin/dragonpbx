...
const digestChallenge = async(req, res, next) => {
  ...
  if (req.locals.authenticated){
    logger.info('call authenticated, skipping digest')
    next() 
  } else {
    try {
      if (!domain) {
        logger.debug('Unknown or invalid realm')
        return res.send(403, {
          headers: {
      ...
      // Auth header without a response (eg first request)
      if (!pieces) return respondChallenge(req, res);
