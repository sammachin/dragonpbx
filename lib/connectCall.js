...
async function getDest(params, domain, rClient) {
    switch (params.type) {
        case 'client':
...
class connection extends Emitter {
    ...
    async action(){
        //dest = await getDest(params, this.req.locals. domain, this.rClient)
        this.logger.info(`Looking up contact for ${this.params.address}`)
        let key = `client:${this.req.locals.domain}:${this.params.address}` // temp
        this.dest = await this.rClient.hGet(key, 'contact') // temp 
