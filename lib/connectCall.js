...
class connection extends Emitter {
    constructor(cs, params, transfer) {
        ...
        this.ringtone = false
        this.sipInfo = false
        this.stolen = false //set true when a pickup verb steals the ringing call
    ...
    async getDestForParam(param) {
        ...
        switch (param.type) {
            case 'client': {
                ...
                if (contacts.length == 0){
                    ...
                    for (const c of contacts) {
                        ...
                        d.dest = cdest;
                        d.clientAddress = param.address; //extension this contact belongs to (for pickup)
            ...
            case 'trunk': {
                ...
                destInfo.fromDomain = trunk.outbound.host;
                ...
                destInfo.timeout = param.timeout || 30;
                // Copy so we don't mutate the cached trunk config, and default to
                // DEFAULT_CODECS when the trunk has none configured.
                let trunkCodecs = Array.isArray(trunk.codecs) ? [...trunk.codecs] : [...DEFAULT_CODECS];
                destInfo.directMedia = (trunkCodecs.indexOf('DIRECT') > 0);
                if (destInfo.directMedia) trunkCodecs.splice(trunkCodecs.indexOf('DIRECT'), 1)
                destInfo.codecs = trunkCodecs
                destInfo.headers = param.headers
    ...
    async action() {
        ...
        if (destinations.length === 0) {
            // No reachable destinations (e.g. client not registered). Without this
            // guard, Promise.race([]) below never settles and the call hangs, so the
            // callScript never advances to any fallback verb (e.g. response 486).
            this.logger.info('No destinations available for connect, skipping to next action');
        ...
        // Media identity of the source leg. Normally the inbound A-leg; on an
        // updateLeg that keeps leg B, cs.activeLeg points the reconnect at B's
        // (fresh) rtpengine session instead.
        const details = this.cs.activeLeg
            ? { 'call-id': this.cs.activeLeg.callId, 'from-tag': this.cs.activeLeg.fromTag }
            : {
        ...
        // Publish this ringing call so a pickup verb in another session can find
        // it. `addresses` are the client extensions being rung (for classic
        // pickup); it may be empty for a trunk/sip-only call, but we still publish
        // so an outbound call can be taken over by its originator. Non-reconnect
        // only, while the A-leg is in early media.
        if (!isReconnect) {
            const destArr = Array.isArray(this.dests) ? this.dests : [this.dests];
            const ringingAddresses = destArr
                .filter(p => p && p.type === 'client' && p.address)
                .map(p => String(p.address));
            this.cs.ringing = { addresses: ringingAddresses, connection: this };
        }

        this.req.once('cancel', () => {
            if (this.isConnected) return;
            this.logger.info('A party cancelled during call setup');
            this.cs.ringing = null;
        ...
        if (!isReconnect) {
            try {
                ...
                // e.g. rtpengine unreachable/timeout. Emit done so the call does not
                // hang and the callScript can advance to a fallback verb.
                this.logger.error(err, 'Failed to prepare media (rtpengine), aborting connect');
                this.emit('done', false);
                this.statusHook.send('connect:complete', this.params);
                return;
            }
    ...
    // Called by a pickup verb (in another CallSession) to steal this
    // still-ringing call. Cancels every ringing B leg and hands ownership of
    // the A-leg (req/res) to the caller. Returns false if the call already
    // connected or was already stolen (lost the race).
    stealForPickup() {
        if (this.isConnected || this.stolen) return false;
        this.stolen = true;
        this.cs.ringing = null;
        this.calls.forEach((call, idx) => {
            if (call && call.reqB && call.status === 'trying') {
                clearTimeout(call.ringTimeout);
                this._cancelWithReason(call.reqB, idx, 200, 'Call picked up elsewhere');
            }
        });
    ...
    async _attemptCall(d, index, details, isReconnect) {
        ...
        if (d.directMedia && this.cs.srcDirectMedia) {
            ...
            // Re-offer the source leg's media. cs.activeLeg (updateLeg keeping
            // leg B) supplies B's SDP/codecs; otherwise it's the A-leg.
            const srcSdp = this.cs.activeLeg ? this.cs.activeLeg.sdp : this.req.body;
            const srcCodecs = this.cs.activeLeg ? this.cs.activeLeg.codecs : this.cs.srcCodecs;
            opts.localSdp = await getSdpB(details, srcSdp, srcCodecs, d.codecs)
        } else {
        ...
        // For trunks, the From host must be the trunk's SIP domain rather than
        // the local server address, otherwise the far end rejects with a 403.
        // Setting headers.From explicitly stops drachtio building a
        // <user@localhost> From that the server rewrites to its own IP.
        if (d.fromDomain) {
            opts.headers.From = opts.callingName
                ? `"${opts.callingName}" <sip:${opts.callingNumber}@${d.fromDomain}>`
                : `<sip:${opts.callingNumber}@${d.fromDomain}>`;
        }
        ...
        if (isReconnect && !this.cs.activeLeg) {
            // Ringback plays on the source leg's existing rtpengine session. The
            // active-leg (leg B) path re-INVITEs B onto a fresh session only when
            // the new destination answers, so there is no ringback for it.
        ...
        return new Promise((resolve, reject) => {
            ...
            const onAnswer = (result) => {
                // A pickup verb stole this ringing call after our INVITE was
                // answered — hang up the late answer, don't bridge it.
                if (this.stolen) {
                    this.logger.info(`Call index ${index} answered after pickup steal; tearing down`)
                    try { isReconnect ? result.destroy() : result.uas.destroy(); } catch (e) {}
                    return;
                }
                this.ringtone.emit('stop');
                ...
                if (!this.isConnected) {
                    ...
                    // No longer ringing — record which client extension answered
                    // and its codecs (for connected-state pickup) and clear the
                    // ringing index.
                    this.cs.ringing = null;
                    this.cs.connectedClient = d.clientAddress || null;
                    this.cs.connectedCodecs = d.codecs || null;
                    resolve(result);
    ...
    _onConnected(result, isReconnect) {
        ...
        // Back-reference so a pickup verb (connected state) can unwind this
        // session's parked run loop after stealing its legs.
        this.cs.activeConnection = this
        ...
        // Signal that this call is now connected. A pickup verb waiting to take
        // over a still-ringing outbound call listens for this to trigger its
        // (connected-state) bridge once the far end answers.
