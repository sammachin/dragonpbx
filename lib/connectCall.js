...
class connection extends Emitter {
        ...
        switch (param.type) {
            case 'client':
                ...
                if (cdest){
                    ...
                    destInfo.directMedia =(clientCodecs.indexOf('DIRECT') > 0);
                    if (destInfo.directMedia) clientCodecs.splice(clientCodecs.indexOf('DIRECT'), 1)
                    destInfo.codecs = clientCodecs;
    ...
    async _attemptCall(d, index, details, isReconnect) {
        ...
        if (!isReconnect) this.ringtone.emit('play');
        ...
        return new Promise((resolve, reject) => {
            const cbRequest = async (err, reqB) => {
                ...
                this.calls[index].ringTimeout = setTimeout(() => {
                    ...
                    if (!isReconnect) this.ringtone.emit('stop');
