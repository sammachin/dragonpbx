...
const connectionMulti = require('./connectCallMulti')
      ...
      return rtpClient.offer(RTPENGINE_PORT, RTPENGINE_HOST, Object.assign(this.details, {'sdp': this.req.body}))
        ...
        .then(() => {
          ...
          if (Object.keys(this.callScript).includes('connect')) {
              //if (this.callScript.connect.length == 1){
                //Single ring
              //  let action = new connection(this, this.callScript.connect[0]);
              //  schedule.push(action);
              //} else{
                //Multi Ring
                let action = new connectionMulti(this, this.callScript.connect);
                schedule.push(action);
              //} 
          }
