...
class CallSession extends Emitter {
  ...
  // leg (keep = 'A' the uas, or 'B' the uac) and terminate the other. Mirrors
  // the REFER/transfer flow in connectCall and runs in reconnect mode.
  //
  // Leg A is the natural case (it is already the uas). For leg B we promote the
  // uac to the controlling leg: describe its media via cs.activeLeg (a fresh
  // rtpengine session), swap the dialog so the kept leg is the uas throughout
  // answer + teardown, and tear down the old A<->B bridge. Leg B has no media
  // until the new destination answers and it is re-INVITEd (no ringback).
  //
  // Returns once the new script has been applied and started; it then runs
  // asynchronously. Throws (with err.code) on validation failures so the caller
  // can map them to HTTP status codes.
  async updateLeg({ keep = 'A', callScript, statusHook } = {}) {
    keep = String(keep).toUpperCase();

    if (keep !== 'A' && keep !== 'B') {
      const err = new Error('leg must be A or B'); err.code = 'BAD_LEG'; throw err;
    }
    ...
    const isB = keep === 'B';
    const keptDialog = isB ? this.dialog.uac : this.dialog.uas;
    const dropDialog = isB ? this.dialog.uas : this.dialog.uac;

    // Resolve leg B's media up front so we can fail cleanly before touching the call.
    let legB = null;
    if (isB) {
      const bTag = keptDialog.sip && keptDialog.sip.remoteTag;
      const bSdp = keptDialog.remote && keptDialog.remote.sdp;
      if (!bTag || !bSdp) {
        const err = new Error('cannot resolve leg B media'); err.code = 'BAD_LEG_STATE'; throw err;
      }
      legB = {
        dialog: keptDialog,
        callId: `${this.req.get('Call-Id')}-legB`,
        fromTag: bTag,
        sdp: bSdp,
        codecs: (Array.isArray(this.connectedCodecs) && this.connectedCodecs.length)
          ? this.connectedCodecs : this.srcCodecs,
      };
    }
    ...
    const oldDetails = this.details;          // original A<->B rtpengine bridge
    ...
    // this.schedule (the array the parked run loop iterates). build() resets
    // this.details from the A-leg, so leg-B overrides happen after it.
    this.callScript = callScript;
    await this.build(true);

    if (isB) {
      // Promote leg B to the controlling leg.
      this.activeLeg = legB;
      this.details = { 'call-id': legB.callId, 'from-tag': legB.fromTag };
      this.dialog = { uas: keptDialog, uac: dropDialog };
    } else {
      this.activeLeg = null;
    }
    ...
    if (isB) { this.uasActive = false; } else { this.uacActive = false; }
    try { dropDialog.destroy(); } catch (e) {}

    // For leg B the original A<->B bridge is now orphaned; the reconnect builds a
    // fresh session for B. (Leg A keeps re-using its own session.)
    if (isB && oldDetails) {
      try { rtpClient.delete(RTPENGINE_PORT, RTPENGINE_HOST, oldDetails); } catch (e) {}
    }
