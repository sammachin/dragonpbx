  ...
  async build(transfer=false) {
    if (this.callScript.length == 1 && this.callScript[0].verb == 'response') {
        ...
        if (this.req.locals.trunk) {
            this.srcDirectMedia = (this.req.locals.trunk.codecs.indexOf('DIRECT') > 0);
            if (this.srcDirectMedia) this.req.locals.trunk.codecs.splice(this.srcDirectMedia, 1)
            ...
            switch (item.verb) {
                case 'announce':
                    try {
                        ...
                        this.logger.error({err: err.message || err, url: item.url},
                            'announce: failed to fetch media, skipping announcement');
                    }
                    break;
                case 'connect':
