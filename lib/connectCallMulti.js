...
const Transfer = require('./transferCall')
...
class connection extends Emitter {
    ...
    async action() {
        if (this.cs.successfullyConnected && !this.reconnect){
            ...
            this.emit('done')
        } else if (this.cs.successfullyConnected && this.reconnect) {
            ...
            Promise.race(b2bPromises)
                .then((dialog) => {
                    ...
                    dialog.uac.on('refer', (req, res) => {
                        ...
                        transfer.prepare(this.cs, req, res, 'uac')
                        res.send(200)
                    })
                    dialog.uas.on('refer', (req, res) => {
                        this.logger.info('REFER Recieved from UAS')
                        req.locals = {logger : this.logger}
                        let transfer = new Transfer()
                        transfer.prepare(this.cs, req, res, 'uas')
                        res.send(200)
                    })
                })
                .catch((err) => {
