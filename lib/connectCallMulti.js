...
class connection extends Emitter {
    ...
    async action() {
        console.log(this.cs.schedule)
        if (this.cs.successfullyConnected && (!this.reconnect || this.cs.transfer)){
            this.logger.info('Skipping action as call had already been connected ')
            this.emit('done')
        } else if (this.cs.successfullyConnected && (this.reconnect || this.cs.transfer)) {
            // this is a reConnect attempt
            this.logger.info('reConnect action')
            ...
            Promise.race(b2bPromises)
                .then((dialog) => {
                    ...
                    dialog.uac.on('refer', (req, res) => {
                        ...
                        .then(result => {
                            console.log('Transfer preparation completed');
                            this.cs.callScript= result.callScript
                            this.cs.transfer = true
                            this.cs.build()
                            .then( () => {
                                res.send(200)
                                this.cs.uacActive=false
