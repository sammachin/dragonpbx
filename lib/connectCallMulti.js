...
class connectionMulti extends Emitter {
    ...
    async action() {
        ...
        const b2bPromises = destinations.map((d, index) => {
            ...
            this.logger.info(`Sending INFITE to  ${d.dest} Index: ${index}`);
                ...
                opts,
            {
                cbRequest: async(err, reqB) => {
                    ...
                    this.b2bCalls[index].ringTimeout = setTimeout(() =>{   
                        this.logger.info(`Cancelling Invite to ${index} due to timeout`)   
                        reqB.cancel()
                    }, ringTimer)
                }
            }
                ...
                if (!this.isConnected) {
                    ...
                    if (idx !== index && call && call.reqB) {
                        this.logger.info('cancelling call attempt Index: ', idx)
                        call.reqB.cancel();
                ...
                .catch((err) => {
                    if (!this.isConnected) {
                        this.logger.error('Error on call index: ', index)
        ...
        Promise.race(b2bPromises)
            ...
            .catch((err) => {
            this.logger.error('All B2BUA attempts failed:', err);1
