...
async function getDomain(domain){
    if (!config.domains || !Object.keys(config.domains).includes(domain)){
        return false;
    } else {
...
async function getRegHook(domain, user){
    ...
    if (reg.hasOwnProperty('reghook')&& reg.reghook != null){
...
async function getTrunks(domain){
    if (!config.domains || !config.domains[domain]) return []
    return config.domains[domain].trunks || []
}

async function getAuthTrunks(domain){
    trunks = []
...
async function getRegTrunks(domain){
    trunks = []
    config.domains[domain].trunks.forEach(t => {
        if (t.authType == 'registration'){
            trunks.push(t)
        }
    })
    return trunks
